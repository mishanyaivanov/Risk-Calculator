...
def evaluate_full_revaluation_stress_scenario(
    ...
    if horizon_days <= 0:
        raise ValueError("Горизонт stress-сценария должен быть > 0 дней.")
    if not positions:
        raise ValueError("Нужна хотя бы одна позиция.")
    ...
    for idx, position in enumerate(positions, start=1):
        ...
        if underlying_id not in underlying_return_shocks:
            raise ValueError(
                f"Для underlying '{underlying_id}' не задан return-shock в stress-сценарии."
            )
