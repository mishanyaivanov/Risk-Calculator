...
def rolling_historical_var_es(
    ...
    if len(pnl) <= window:
        raise ValueError("P&L series length must be greater than the rolling backtest window.")
    if window < 20:
        raise ValueError("Backtest window must be at least 20 observations.")
...
def kupiec_pof_test(exceptions: list[bool], alpha: float) -> dict[str, float | int | str]:
    if not exceptions:
        raise ValueError("Kupiec test requires a non-empty exceptions series.")
...
def christoffersen_independence_test(
    ...
    if len(exceptions) < 2:
        raise ValueError("Christoffersen independence test requires at least 2 observations.")
    ...
    if total == 0:
        raise ValueError("Not enough data to estimate exception state transitions.")
...
def es_realized_shortfall_diagnostics(
    ...
    if not realized_pnl:
        raise ValueError("Realized P&L series must not be empty.")
    if len(realized_pnl) != len(var_pnl_thresholds) or len(realized_pnl) != len(es_losses):
        raise ValueError("Series lengths for ES diagnostics must match.")
